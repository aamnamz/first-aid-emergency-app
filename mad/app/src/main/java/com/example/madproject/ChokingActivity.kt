...
class ChokingActivity : AppCompatActivity(), OnInitListener {
    ...
    private val UTTERANCE_ID = "chokingInstructions" // Added Utterance ID constant

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_choking) // Assuming your layout is named activity_choking
        ...
        tts.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
            ...
            override fun onError(utteranceId: String?) {
                if (utteranceId == UTTERANCE_ID) {
                    runOnUiThread {
                        btnSpeak.text = SPEAK_TEXT
                        Log.e("TTS", "Speech error in ChokingActivity.")
                    }
                }
            }
            ...
            override fun onStop(utteranceId: String?, interrupted: Boolean) {
                runOnUiThread {
                    btnSpeak.text = SPEAK_TEXT
                    Log.d("TTS", "Speech stopped by user interaction in ChokingActivity.")
                }
            }
        })
    }
    ...
    private fun extractInstructionsFromLayout(): String {
        ...
        for (i in 0 until instructionContainer.childCount) {
            ...
            if (view is TextView) {
                ...
                // Changed heuristic to generic "First Aid" check for safety
    ...
    override fun onInit(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            ...
            if (result == TextToSpeech.LANG_MISSING_DATA || result == TextToSpeech.LANG_NOT_SUPPORTED) {
                Log.e("TTS", "Language is not supported or missing data in ChokingActivity.")
                ...
                Log.d("TTS", "TTS Initialization successful in ChokingActivity.")
            }
        } else {
            Log.e("TTS", "Initialization failed in ChokingActivity.")
    ...
    override fun onDestroy() {
        if (::tts.isInitialized) { // Added ::tts.isInitialized check for safety
            tts.stop()
            tts.shutdown()
        }
        super.onDestroy()
